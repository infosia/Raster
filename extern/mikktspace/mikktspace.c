...
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const SMikkTSpaceContext * pContext, const int iNrTrianglesIn)
{
	...
	for (k=0; k<g_iCells; k++)
	{
		...
		if (pTmpVert!=NULL)
		{
			...
			{
				int l = pTable[e];
				const SVec3 vP = GetPosition(pContext, piTriList_in_and_out[l]);
				pTmpVert[e].vert[0] = vP.x; pTmpVert[e].vert[1] = vP.y;
				pTmpVert[e].vert[2] = vP.z; pTmpVert[e].index = l;
			}
		...
		for (i=0; i<pGroup->iNrFaces; i++)	// triangles
		{
			...
			if (!bFound)
			{
				...
				if (pIndices==NULL)
				{
					// clean up and return false
					int s0=0;
					for (s0=0; s0<iUniqueSubGroups; s0++)
						free(pUniSubGroups[s0].pTriMembers);
					free(pUniSubGroups);
					free(pTmpMembers);
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
	...
	for (i=0; i<iEntries; i++)
	{
		...
		const int f0 = pEdges[i].f;
		tbool bUnassigned_A;

		int i0_A, i1_A;
		int edgenum_A, edgenum_B=0;	// 0,1 or 2
		GetEdge(&i0_A, &i1_A, &edgenum_A, &piTriListIn[f0*3], i0, i1);	// resolve index ordering and edge_num
		bUnassigned_A = pTriInfos[f0].FaceNeighbors[edgenum_A] == -1 ? TTRUE : TFALSE;
		...
		if (bUnassigned_A)
		{
			...
			{
				int t0 = pEdges[j].f;
				pTriInfos[f0].FaceNeighbors[edgenum_A] = t0;
				//assert(pTriInfos[t].FaceNeighbors[edgenum_B]==-1);
				pTriInfos[t0].FaceNeighbors[edgenum_B] = f0;
			}
		}
	}
}
...
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris)
{
	...
	while (t<iNrTrianglesIn && bStillFindingGoodOnes)
	{
		...
		else
		{
			...
			{
				const tbool bIsGood0 = (pTriInfos[iNextGoodTriangleSearchIndex].iFlag&MARK_DEGENERATE)==0 ? TTRUE : TFALSE;
				if (bIsGood0) bJustADegenerate=TFALSE;
